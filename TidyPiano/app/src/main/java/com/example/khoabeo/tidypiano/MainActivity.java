...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        int lastPointerId = event.getPointerId(lastPointerIndex);
        ...
        for (int pIndex = 0; pIndex < event.getPointerCount(); pIndex++) {
            ...
            for (int i = 0; i < pressedKeyInfos.size(); i++) {
                ...
                if ((!isInside(pX, pY, pressedKeyInfo.getIvKey()) && pointerAction == MotionEvent.ACTION_MOVE)
                        || (isInside(pX, pY, pressedKeyInfo.getIvKey())
                        && (pointerAction == MotionEvent.ACTION_POINTER_UP || pointerAction == MotionEvent.ACTION_UP)
                        && pId == lastPointerId)) {
